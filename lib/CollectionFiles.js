...
let imagesBucket
if (Meteor.isServer) {
  imagesBucket = createBucket('files')
}

this.CollectionFiles = new FilesCollection({
  collectionName: 'files',
  storagePath: () => {
    return `${process.env.PWD}/uploadedFiles`;
  },
  ...
  interceptDownload(http, file, versionName) {
    ...
    if (gridFsFileId) {
      ...
      http.response.setHeader('Content-Disposition', `inline; filename="${encodeURIComponent(file.name)}"`);
    }
...
if (Meteor.isClient) {
  CollectionFiles.insertFile = (file, meta) => {
    if (!meta) {
      meta = {}
    }
    return CollectionFiles.insert({ file: file, meta: meta }).config.fileId
  }
  // Meteor.subscribe('CollectionFiles');
}

if (Meteor.isServer) {
  // Meteor.publish('CollectionFiles', () => CollectionFiles.collection.find({}));
}
// export { CollectionFiles }

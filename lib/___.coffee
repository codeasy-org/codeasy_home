...
#이메일 형식 check / return true, false
    if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(mail)) then return true else return false
...
#obj, arr, string 등 값의 존재 유무 확인
    if typeof _obj is 'number' then return true else if typeof _obj is 'boolean' then return true else return _obj? and !!_obj and !!Object.keys(_obj).length
...
#    else !arr.includes(false) #and
  ...
  hasOnlyDigits: (_val) ->
    if /^-?\d+\.?\d*$/.test _val then return true else return false
  getClassName: (obj) ->
    if typeof obj is "undefined" then return "undefined"
    if obj is null then return "null"
    return Object.prototype.toString.call(obj).match(/^\[object\s(.*)\]$/)[1]
  ConvertToWei: (_balance) ->
    return new BigNumber(_balance).multipliedBy(new BigNumber(10).pow(18))
  ConvertToEther: (_balance) ->
    return new BigNumber(_balance).div(new BigNumber(10).pow(18)).toString(10)
  CompareBalance: (_myBalance,_sellPrice) ->
    return new BigNumber(_myBalance).gte(_sellPrice)
  ETHPrice: ->
    currentPrice = await fetch('https://api2.binance.com/api/v3/avgPrice?symbol=ETHUSDT',{method:'get'})
    return await currentPrice.json()